#!/usr/bin/env bash
# Cover Letter Skill — Universal Installer
# Auto-detects: Claude Code, Cursor, Cline, Windsurf, Copilot, Aider, Generic
# Works locally OR piped:  curl -fsSL <raw>/install.sh | bash

set -eo pipefail

SKILL_NAME="krompt-cover-letter"
SKILL_VERSION="1.0.0"
SKILL_DESC="Write cover letters in your own voice — captures tone and structure, never bakes in projects or employers. Modes: application, research, scholarship."
SKILL_TRIGGERS="write a cover letter|draft a cover letter|use the cover-letter skill|write a cover letter in my tone|research cover letter|scholarship essay"
...
main() {
  ...
  log "Modes: application (default), research, scholarship"
}
main
