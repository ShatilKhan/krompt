...
SKILL_NAME="krompt-deployment-pipeline"
SKILL_VERSION="1.0.0"
SKILL_DESC="Generate complete deployment pipelines (GH Actions + Dockerfile + compose + Caddyfile + migration scaffolds). Cloud-provider agnostic. CI-driven or git-auto-deploy."
SKILL_TRIGGERS="generate a deployment pipeline|set up deployment|create a CI/CD workflow|deploy to vercel and turso|deploy to ec2 with docker|deploy to digitalocean|scaffold a Dockerfile and compose"
RAW_BASE="${KROMPT_RAW_BASE:-https://raw.githubusercontent.com/ShatilKhan/krompt/main/skills/${SKILL_NAME}}"
SOURCE_FILE="prompt.md"

# Full template tree — paths are relative to skill's templates/ dir.
TEMPLATE_PATHS=(
  "github-actions/deploy-ssh-host-registry-echo-loop.yml"
  "github-actions/deploy-ssh-host-registry-branch-env.yml"
  "github-actions/deploy-ssh-host-source-heredoc.yml"
  "github-actions/deploy-do-app-platform.yml"
  "github-actions/deploy-aws-ecs.yml"
  "github-actions/deploy-cloud-run.yml"
  "vercel/vercel.json"
  "vercel/api-handler-example.ts"
  "vercel/lib-db-turso.ts"
  "vercel/lib-db-drizzle-libsql.ts"
  "fly/fly.toml"
  "railway/railway.json"
  "railway/nixpacks.toml"
  "render/render.yaml"
  "docker/Dockerfile.node-multistage"
  "docker/Dockerfile.bun-multistage"
  "docker/Dockerfile.python-slim"
  "docker/docker-compose.base.yml"
  "docker/docker-compose.with-migrate.yml"
  "migrations/drizzle/drizzle.config.ts"
  "migrations/drizzle/migrate-postgres-js.ts"
  "migrations/prisma/schema.prisma"
  "migrations/alembic/alembic.ini"
  "migrations/alembic/env.py"
  "migrations/dbmate/compose-migrate-service.yml"
  "migrations/golang-migrate/compose-migrate-service.yml"
  "migrations/raw-psql/migrate.sh"
  "migrations/raw-psql/0000_init_schema_migrations_table.sql"
  "caddy/Caddyfile"
  "do-app-platform/deploy.template.yaml"
)
...
copy_templates() {
  ...
  for rel in "${TEMPLATE_PATHS[@]}"; do
    local dest="$dest_dir/templates/$rel"
    mkdir -p "$(dirname "$dest")"
    if [[ -n "$dir" && -f "$dir/templates/$rel" ]]; then
      cp "$dir/templates/$rel" "$dest"
    else
      fetch "$RAW_BASE/templates/$rel" > "$dest" 2>/dev/null || { warn "skip (404 or fetch failed): $rel"; rm -f "$dest"; continue; }
    fi
    count=$((count + 1))
  done
  ok "Templates → $dest_dir/templates/ ($count files)"
}
...
main() {
  ...
  [[ "${WITH_TEMPLATES:-0}" != "1" ]] && log "Tip: re-run with --with-templates to also copy the full templates tree (workflows, Dockerfiles, compose, Caddyfile, migration scaffolds)."
}
main
